...
train_data_loader_accuracy = DataLoader(train_dataset,batch_size=100, shuffle=True)
test_dataset = CustomDataset("MMAFEDB/test/")
test_data_loader = DataLoader(train_dataset, batch_size=100, shuffle=True)
val_dataset = CustomDataset("MMAFEDB/valid/")
val_data_loader = DataLoader(train_dataset, batch_size=100,shuffle=True)
...
class FaceEmotionModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.predef_model=models.vgg16()
    ...
    def forward(self,X):  
        return self.last_layer(self.predef_model(X.float()))
